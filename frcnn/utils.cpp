...
namespace frcnn
{
  ...
  torch::Tensor bbox_center(torch::Tensor bboxes){
    return torch::stack({(bboxes.index({Slice(), 0}) + bboxes.index({Slice(), 2}))/2,
	  (bboxes.index({Slice(), 1}) + bboxes.index({Slice(), 3}))/2}, 1);
  }
  ...
  void ProgressTracker::report_progress(std::ostream &os){
    ...
    for (auto &loss_key : {"loss_cls", "loss_bbox", "loss_centerness", "loss"}){
  ...
  void tensor_shape(torch::Tensor tsr, const std::string &name){
    std::cout << name << ": " << tsr.sizes() << std::endl;
  }
  void tensor_shape(std::vector<torch::Tensor> tsrs, const std::string &name){
    std::cout << name << ":\n";
    for (auto &tsr : tsrs){
      std::cout << "  " << tsr.sizes() << "\n";
    }
  }
  void tensor_shape(std::vector<std::vector<torch::Tensor>> tsrss, const std::string &name){
    std::cout << name << ":\n";
    for (auto &tsrs : tsrss){
      std::cout << "---\n";
      for (auto &tsr : tsrs){
